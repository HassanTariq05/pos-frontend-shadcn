...
import { vendorAccountColumns as columns } from './vendor-account-columns'

const route = getRouteApi('/_authenticated/business-accounts')
...
export function VendorAccounts({
  data,
  page,
  pageSize,
  totalPages,
  ...
  return (
    <div className={cn('flex flex-1 flex-col gap-4')}>
      <div className='flex flex-row items-center gap-4'>
        <h4 className='text-2xl font-bold tracking-tight'>Vendor Accounts</h4>
        <Badge
          variant='outline'
          className={
                ...
                <TableRow key={`skeleton-${index}`}>
                  {columns.map((column, colIndex) => (
                    ...
                    >
                      <div
                        className={cn(
                          'h-6 w-full animate-pulse rounded-md bg-muted/70',
                          colIndex === 0 && 'w-3/4',
                          colIndex === 1 && 'w-1/2',
